...
const char * const BITCOIN_CONF_FILENAME = "corez.conf";
const char * const BITCOIN_PID_FILENAME = "corezd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "corez" is a composite category enabling all CoreZ-related debug output
            if(ptrCategory->count(string("corez"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CoreZCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CoreZCore
    // Mac: ~/Library/Application Support/CoreZCore
    // Unix: ~/.corezcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CoreZCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CoreZCore";
#else
    // Unix
    return pathRet / ".corezcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty corez.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override corez.conf
